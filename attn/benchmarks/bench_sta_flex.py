...
attn_test_dir = os.path.join(os.path.dirname(current_dir), "tests")
sys.path.insert(0, attn_test_dir)
...
def benchmark_attention(configurations):
    ...
    for B, H, N, D, causal, dit_seq_shape, window_size in configurations:
        ...
        mask = get_sliding_tile_attention_mask(window_size, (6, 8, 8), dit_seq_shape_mapping[dit_seq_shape], 0, 'cuda', 0)
        ms = do_bench(lambda: flex_attention(q, k, v, block_mask=mask))
